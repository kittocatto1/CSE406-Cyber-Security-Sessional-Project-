...
Usage:
    sudo ./run_attack1_wifi.sh <client_port>
...
step "Locating attack1_reset_wifi.py"

ATTACK_SCRIPT="$SCRIPT_DIR/attack1_reset_wifi.py"

if [[ ! -f "$ATTACK_SCRIPT" ]]; then
    fail "attack1_reset_wifi.py not found in $SCRIPT_DIR."
fi
...
python3 "$ATTACK_SCRIPT" "$CLIENT_PORT" "$CLIENT_IP" "$SERVER_IP"
...
if [[ $STATUS -ne 0 ]]; then
    fail "attack1_reset_wifi.py exited with status $STATUS."
fi

ok "attack script finished"
echo "⦿ Check the iperf3 client's terminal on the other machine for the reset."